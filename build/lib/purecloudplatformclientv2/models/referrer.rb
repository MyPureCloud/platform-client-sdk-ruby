...
module PureCloud
  class Referrer
    ...
    def valid?
      ...
      if @medium && !allowed_values.include?(@medium)
        return false
      end
    ...
    def medium=(medium)
      allowed_values = ["internal", "search", "social", "email", "unknown", "paid"]
      if medium && !allowed_values.include?(medium)
        fail ArgumentError, "invalid value for 'medium', must be one of #{allowed_values}."
      end
      @medium = medium
    end
    ...
    def hash
      [url, domain, hostname, keywords, pathname, query_string, fragment, name, medium].hash
    end
