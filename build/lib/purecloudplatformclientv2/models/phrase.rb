...
module PureCloud
  class Phrase
    ...
    def valid?
      ...
      if @sentiment && !allowed_values.include?(@sentiment)
        return false
      end
    ...
    def sentiment=(sentiment)
      allowed_values = ["Unspecified", "Positive", "Neutral", "Negative"]
      if sentiment && !allowed_values.include?(sentiment)
        fail ArgumentError, "invalid value for 'sentiment', must be one of #{allowed_values}."
      end
      @sentiment = sentiment
    end
    ...
    def hash
      [text, strictness, sentiment].hash
    end
