...
describe PureCloud::ApiClient do
  context 'initialization' do
    context 'URL stuff' do
      context 'host' do
        it 'removes http from host' do
          PureCloud.configure { |c| c.host = 'http://example.com' }
          expect(PureCloud::Configuration.default.host).to eq('example.com')
        end

        it 'removes https from host' do
          PureCloud.configure { |c| c.host = 'https://wookiee.com' }
          expect(PureCloud::ApiClient.default.config.host).to eq('wookiee.com')
        end

        it 'removes trailing path from host' do
          PureCloud.configure { |c| c.host = 'hobo.com/v4' }
          expect(PureCloud::Configuration.default.host).to eq('hobo.com')
        end
      end

      context 'base_path' do
        it "prepends a slash to base_path" do
          PureCloud.configure { |c| c.base_path = 'v4/dog' }
          expect(PureCloud::Configuration.default.base_path).to eq('/v4/dog')
        end
        ...
        it "ends up as a blank string if nil" do
          PureCloud.configure { |c| c.base_path = nil }
          expect(PureCloud::Configuration.default.base_path).to eq('')
        end
      end
    end
  end
  ...
  describe "#update_params_for_auth!" do
    it "sets header api-key parameter with prefix" do
      ...
      api_client = PureCloud::ApiClient.new

      config2 = PureCloud::Configuration.new do |c|
        c.api_key_prefix['api_key'] = 'PREFIX2'
        c.api_key['api_key'] = 'special-key2'
      end
      api_client2 = PureCloud::ApiClient.new(config2)
  ...
  describe "timeout in #build_request" do
    let(:config) { PureCloud::Configuration.new }
    let(:api_client) { PureCloud::ApiClient.new(config) }

    it "defaults to 0" do
      expect(PureCloud::Configuration.default.timeout).to eq(0)
  ...
  describe "#deserialize" do
    ...
    it "handles Hash<String, Pet>" do
      ...
      pet = data[:pet]
      expect(pet).to be_instance_of(PureCloud::Pet)
  ...
  describe "#build_collection_param" do
    let(:param) { ['aa', 'bb', 'cc'] }
    let(:api_client) { PureCloud::ApiClient.new }
